import { d as createAstro, c as createComponent, m as maybeRenderHead, r as renderComponent, a as renderTemplate, e as Fragment$1, b as addAttribute, u as unescapeHTML, k as renderScript, f as defineScriptVars, s as spreadAttributes, l as defineStyleVars, n as renderHead, p as renderSlot } from './3oDUBBJB.js';
import 'piccolore';
import * as LucideIcons from 'lucide-react';
import { Menu, X } from 'lucide-react';
import { jsx, Fragment } from 'react/jsx-runtime';
import { S as SITE, b as getHomePermalink, A as ANALYTICS, U as UI, c as getAsset, g as getCanonical, I as I18N, M as METADATA, a as getPermalink, d as cn } from './CS4DDvGF.js';
/* empty css         */
import 'clsx';
import { escape } from 'html-escaper';
import merge from 'lodash.merge';
import { g as getImage } from './C_qj9URY.js';
...
const $$Footer = createComponent(($$result, $$props, $$slots) => {
  ...
  return renderTemplate`${maybeRenderHead()}<footer${addAttribute([{ dark: theme === "dark" }, "not-prose relative border-t border-gray-200 dark:border-slate-800 bg-white dark:bg-dark"], "class:list")}> <div class="dark:bg-dark pointer-events-none absolute inset-0" aria-hidden="true"></div> <div class="mx-auto w-full"> <div class="px-6 py-8 xl:px-8"> <div class="columns-1 gap-6 sm:columns-2 lg:columns-3 xl:columns-4"> ${links.map(({ title, items }) => renderTemplate`<div class="mb-6 break-inside-avoid"> <h2 class="mb-3 text-sm font-semibold text-gray-900 dark:text-white">${title}</h2> ${items && Array.isArray(items) && items.length > 0 && renderTemplate`<div class="flex flex-wrap gap-2"> ${items.map(({ title: text, href, ariaLabel }) => renderTemplate`<a class="inline-block rounded-md border border-gray-200 bg-gray-50 px-2.5 py-1 text-xs text-gray-700 transition-all duration-200 hover:border-orange-300 hover:bg-orange-100 hover:text-orange-600 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-300 dark:hover:border-orange-500 dark:hover:bg-orange-900/20 dark:hover:text-orange-400"${addAttribute(href, "href")}${addAttribute(ariaLabel, "aria-label")} target="_blank"${addAttribute(text, "title")}> ${renderComponent($$result, "Fragment", Fragment$1, {}, { "default": ($$result2) => renderTemplate`${unescapeHTML(text)}` })} </a>`)} </div>`} </div>`)} <!-- <div>
      ...
      </div> --> </div> </div> <div class="bg-gray-100 px-4 py-8 md:py-2 dark:bg-gray-700"> <div class="mx-auto w-full md:flex md:items-center md:justify-between xl:w-[1400px]"> <span class="text-sm text-gray-500 sm:text-center dark:text-gray-300">© 2023 <a${addAttribute(getHomePermalink(), "href")}>${SITE?.name}</a> - <a class="mr-2" href="/phien-ban-cap-nhat">Phiên bản ${VERSION_HORO}</a> ${renderComponent($$result, "OnlineView", null, { "client:only": "react", "client:component-hydration": "only", "client:component-path": "/root/code/tmd_astro/src/components/common/OnlineView", "client:component-export": "OnlineView" })} </span> <div class="mt-4 flex space-x-5 sm:justify-center md:mt-0 rtl:space-x-reverse"> ${socialLinks?.length ? renderTemplate`<ul class="mb-4 -ml-2 flex md:order-1 md:mb-0 md:ml-4 rtl:-mr-2 rtl:ml-0 md:rtl:mr-4 md:rtl:ml-0"> ${socialLinks.map(({ ariaLabel, href, title: text, icon }) => {
  ...
  links: [
    {
      ...
      items: [
        {
          title: 'Tử vi & tứ trụ theo vị trí sinh',
    ...
    },
    {
      ...
      items: [
        ...
        { title: 'Xem tuổi xông đất 2026', href: '/xem-tuoi-xong-dat-nam-2026-binh-ngo', icon: 'CalendarCheck' },
