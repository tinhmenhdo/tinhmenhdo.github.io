import { b as createAstro, c as createComponent, m as maybeRenderHead, e as addAttribute, a as renderTemplate, r as renderComponent, d as Fragment$1, u as unescapeHTML, j as renderScript, k as defineScriptVars, s as spreadAttributes, l as defineStyleVars, n as renderSlot, p as renderHead } from './uu9o3mPN.js';
import 'piccolore';
import * as LucideIcons from 'lucide-react';
import { Compass, MessageCircleQuestion, Menu } from 'lucide-react';
import { c as cn, S as SITE, a as getHomePermalink, A as ANALYTICS, U as UI, b as getAsset, g as getCanonical, I as I18N, M as METADATA, d as getPermalink } from './D5X8WKes.js';
import { jsx, Fragment } from 'react/jsx-runtime';
import 'clsx';
/* empty css         */
import { escape } from 'html-escaper';
import merge from 'lodash.merge';
import { g as getImage } from './je4qbpvC.js';
import { parseUrl, transformUrl } from 'unpic';

const logo = new Proxy({"src":"/images/favicon-32x32.png","width":32,"height":32,"format":"png"}, {
						get(target, name, receiver) {
							if (name === 'clone') {
								return structuredClone(target);
							}
							if (name === 'fsPath') {
								return "/root/code/tmd_astro/src/assets/favicons/favicon-32x32.png";
							}
							if (target[name] !== undefined && globalThis.astroAsset) globalThis.astroAsset?.referencedImages.add("/root/code/tmd_astro/src/assets/favicons/favicon-32x32.png");
							return target[name];
						}
					});
...
const $$BottomNav = createComponent(($$result, $$props, $$slots) => {
  ...
  return renderTemplate`${maybeRenderHead()}<div class="pointer-events-none fixed right-0 bottom-4 left-0 z-50 flex justify-center px-4"> <nav class="liquid-glass-bg px-0.7 pointer-events-auto relative flex h-14 w-fit min-w-80 items-center justify-around gap-1 rounded-full backdrop-blur-md"> ${navItems.map((item) => {
    ...
    ), "class")}> ${item.isActive ? renderTemplate`<div class="liquid-glass-active absolute top-1/2 left-1/2 flex h-15 min-w-24 -translate-x-1/2 -translate-y-1/2 flex-col items-center justify-center rounded-[1.75rem]"> ${item.imageSrc ? renderTemplate`<img${addAttribute(item.imageSrc, "src")}${addAttribute(item.label, "alt")} class="mb-0.5 h-[19px] w-[19px] object-contain drop-shadow-[0_0_10px_rgba(239,68,68,0.8)]">` : Icon && renderTemplate`${renderComponent($$result, "Icon", Icon, { "className": "mb-0.5 h-5 w-5 text-red-600 drop-shadow-[0_0_6px_rgba(239,68,68,0.5)]" })}`} <span class="text-[10px] font-bold text-red-600">${item.label}</span> </div>` : renderTemplate`<div class="flex flex-col items-center justify-center text-orange-400/80 transition-colors group-hover:text-red-600"> ${item.imageSrc ? renderTemplate`<img${addAttribute(item.imageSrc, "src")}${addAttribute(item.label, "alt")} class="mb-0.5 h-[19px] w-[19px] object-contain opacity-70 grayscale transition-all group-hover:opacity-100 group-hover:grayscale-0">` : Icon && renderTemplate`${renderComponent($$result, "Icon", Icon, { "className": "mb-0.5 h-5 w-5" })}`} <span class="text-[10px] font-medium text-stone-500 transition-colors group-hover:text-red-600"> ${item.label} </span> </div>`} </a>`;
  })} </nav> </div>`;
...
const $$Layout = createComponent(($$result, $$props, $$slots) => {
  ...
  return renderTemplate`<html${addAttribute(language, "lang")}${addAttribute(textDirection, "dir")}${addAttribute(initialLayout, "data-layout")}${addAttribute($$definedVars, "style")}> <head>${renderComponent($$result, "CommonMeta", $$CommonMeta, {})}${renderComponent($$result, "Favicons", $$Favicons, {})}${renderComponent($$result, "Metadata", $$Metadata, { ...metadata })}${renderComponent($$result, "SiteVerification", $$SiteVerification, {})}${renderComponent($$result, "Analytics", $$Analytics, {})}${renderComponent($$result, "ClientRouter", $$ClientRouter, { "fallback": "animate" })}${renderSlot($$result, $$slots["head"])}${renderHead()}</head> <body${addAttribute(`min-h-screen bg-background font-sans antialiased overflow-x-hidden bg-body relative ${isHome ? "is-home" : "not-home"}`, "class")}${addAttribute($$definedVars, "style")}> ${renderSlot($$result, $$slots["default"])} ${renderComponent($$result, "BasicScripts", $$BasicScripts, {})} ${renderComponent($$result, "VersionChecker", null, { "client:only": "react", "client:component-hydration": "only", "client:component-path": "~/components/common/VersionChecker", "client:component-export": "default" })}  </body> </html>`;
