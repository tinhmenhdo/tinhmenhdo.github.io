import { b as createAstro, c as createComponent, m as maybeRenderHead, u as unescapeHTML, a as renderTemplate, e as addAttribute, r as renderComponent, d as Fragment } from './F_Fel0yb.js';
import 'piccolore';
import 'clsx';
import { $ as $$Icon } from './CPkmLE2Q.js';
import { $ as $$Image, a as $$Tags, b as $$Button } from './BffexApU.js';
import { f as findImage } from './Dks9rDWI.js';
import { d as getPermalink, j as getFormattedDate } from './Du4CJw7g.js';
...
const $$ListItem = createComponent(async ($$result, $$props, $$slots) => {
  ...
  return renderTemplate`${maybeRenderHead()}<article${addAttribute(`max-w-md mx-auto md:max-w-none grid gap-6 md:gap-8   ${image || post.imageRaw ? "md:grid-cols-2" : ""}`, "class")}> ${(image || post.imageRaw) && (link ? renderTemplate`<a class="group relative block"${addAttribute(link ?? "javascript:void(0)", "href")}> <div class="relative h-0 overflow-hidden rounded pb-[56.25%] shadow-lg md:h-72 md:pb-[75%] lg:pb-[56.25%]"> ${post.imageRaw ? renderTemplate`<img${addAttribute(post.imageRaw, "src")} class="absolute inset-0 mb-6 h-full w-full rounded bg-gray-400 object-cover shadow-lg"${addAttribute(post.title, "alt")} loading="lazy">` : image && renderTemplate`${renderComponent($$result, "Image", $$Image, { "src": image, "class": "absolute inset-0 mb-6 h-full w-full rounded bg-gray-400 object-cover shadow-lg", "widths": [400, 900], "width": 900, "alt": post.title, "aspectRatio": "16:9", "loading": "lazy", "decoding": "async" })}`} </div> </a>` : renderTemplate`<div class="relative h-0 overflow-hidden rounded bg-gray-400 pb-[56.25%] shadow-lg md:h-72 md:pb-[75%] lg:pb-[56.25%]"> ${post.imageRaw ? renderTemplate`<img${addAttribute(post.imageRaw, "src")} class="absolute inset-0 mb-6 h-full w-full rounded bg-gray-400 object-cover shadow-lg"${addAttribute(post.title, "alt")} loading="lazy">` : image && renderTemplate`${renderComponent($$result, "Image", $$Image, { "src": image, "class": "absolute inset-0 mb-6 h-full w-full rounded bg-gray-400 object-cover shadow-lg", "widths": [400, 900], "width": 900, "alt": post.title, "aspectRatio": "16:9", "loading": "lazy", "decoding": "async" })}`} </div>`)} <div class="mt-2"> <header> <div class="mb-1"> <span class="text-sm"> ${renderComponent($$result, "Icon", $$Icon, { "name": "tabler:clock", "class": "-mt-0.5 inline-block h-3.5 w-3.5" })} <time${addAttribute(String(post.publishDate), "datetime")} class="inline-block">${getFormattedDate(post.publishDate)}</time> ${post.author && renderTemplate`${renderComponent($$result, "Fragment", Fragment, {}, { "default": async ($$result2) => renderTemplate`${" "}
· ${renderComponent($$result2, "Icon", $$Icon, { "name": "tabler:user", "class": "-mt-0.5 inline-block h-3.5 w-3.5" })} <span>${post.author.replaceAll("-", " ")}</span> ` })}`} ${post.category && renderTemplate`${renderComponent($$result, "Fragment", Fragment, {}, { "default": async ($$result2) => renderTemplate`${" "}
·${" "}<a class="hover:underline"${addAttribute(getPermalink(post.category.slug, "category"), "href")}> ${post.category.title} </a> ` })}`} </span> </div> <h2 class="font-heading mb-2 text-xl leading-tight font-bold sm:text-2xl"> ${link ? renderTemplate`<a class="hover:text-primary inline-block transition duration-200 ease-in"${addAttribute(link, "href")}> ${post.title} </a>` : post.title} </h2> </header> ${post.excerpt && renderTemplate`<p class="text-muted grow text-lg">${post.excerpt}</p>`} ${post.tags && Array.isArray(post.tags) ? renderTemplate`<footer class="mt-5"> ${renderComponent($$result, "PostTags", $$Tags, { "tags": post.tags })} </footer>` : renderTemplate`${renderComponent($$result, "Fragment", Fragment, {})}`} </div> </article>`;
