...
import { C as CHI } from './Dc1Oa40J.js';
...
const CHART_CONTAINER_CLASSES = "relative mx-auto mb-6 h-[500px] max-h-[95vw] w-[500px] max-w-[95vw] rounded-2xl border border-gray-200 bg-gradient-to-br from-white to-gray-50 shadow-xl max-md:-ml-2";
const QUESTION_PANEL_CLASSES = "w-full max-w-[250px] rounded-xl border border-blue-200 bg-gradient-to-br from-white to-blue-50 p-2 font-medium shadow-xl backdrop-blur-sm";
const createGetCungClassName = (colors) => (cung, isActive, currentCung, hasContent = false) => {
  const baseClasses = `absolute w-[22%] h-[22%] ${colors[cung].bg} p-3 max-md:p-2 rounded-xl shadow-xl border border-gray/20 transition-all duration-300 hover:shadow-2xl hover:scale-105`;
...
const createGetCungClassNameClickable = (colors) => (cung, hasStars) => {
  const baseClasses = `absolute w-[22%] h-[22%] rounded-2xl ${colors[cung].bg} p-3 max-md:p-2 shadow-xl border border-gray/20 transition-all duration-300 hover:shadow-2xl hover:scale-105 cursor-pointer hover:bg-yellow-50`;
